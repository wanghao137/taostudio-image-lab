export interface ParsedSkillMarkdown {
  name: string
  /** 可选中文标题（frontmatter title）；未设置或空串时为 undefined，展示层回落 name */
  title?: string
  description: string
  body: string
}
 ...
 *  另取自研扩展 title 作中文展示标题，其余忽略）；无 frontmatter 容错降级。不引入 YAML 依赖。 */
export function parseSkillMarkdown(text: string, fallbackName: string): ParsedSkillMarkdown | null {
  ...
  return { name: name || fallbackName, title: title || undefined, description, body: body.trim() || normalized }
}
