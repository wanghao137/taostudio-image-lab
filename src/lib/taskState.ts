...
export function resolveFinalActualParams(
  params: ActualParams | undefined,
  size: ImageSize | undefined,
  taskParams: Pick<TaskParams, 'exact_size'>,
...
export function deriveFinalGalleryActualParams(
  provider: ApiProvider,
  isAsyncCustomTask: boolean,
  resultParams: ActualParams | undefined,
  paramsList: Array<ActualParams | undefined> | undefined,
  ...
  const derived = deriveGalleryActualParams(provider, isAsyncCustomTask, resultParams, paramsList, outputCount)
  if (!taskParams.exact_size || !derived) return derived
  const actualSize = firstActualParams(paramsList)?.size
  return actualSize ? { ...derived, size: actualSize } : derived
}
