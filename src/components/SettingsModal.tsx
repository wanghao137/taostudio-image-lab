...
function isPristineNewOpenAIProfile(profile: ApiProfile) {
  ...
  // 升级前持久化的 profile 该字段归一化为 ''，要求等于默认值会让存量「新配置」永不 pristine。
...
export default function SettingsModal() {
  ...
  const commitSettings = (nextDraft: AppSettings) => {
    const normalizedProfiles = nextDraft.profiles.map((profile) => {
      ...
      const defaultModel = profile.provider === 'fal' ? DEFAULT_FAL_MODEL : getDefaultOpenAIModel(profile.apiMode)
      return {
        ...profile,
  ...
  const createProfileImportUrl = (profile: ApiProfile, options: ProfileImportUrlOptions) => {
    ...
    if (profile.provider === 'openai') {
      ...
      const model = profile.model.trim() || getDefaultOpenAIModel(profile.apiMode)
  ...
  return (
        <div
          data-no-drag-select
          className="fixed inset-0 z-[70] flex items-center justify-center p-4 max-sm:items-end max-sm:p-0"
            ...
            {activeTab === 'api' && (
              ...
              {(activeProfile.apiMode ?? DEFAULT_SETTINGS.apiMode) === 'responses' && activeProfile.provider === 'openai' && (
                <div className="block">
                  ...
                  <div data-selectable-text className="mt-1.5 text-xs text-gray-500 dark:text-gray-500">
                    用于指导模型在执行任务时的思考深度，更高的档位会使模型花费更长时间进行思考，有助于提升模型完成复杂任务的能力。并非所有模型都支持全部推理强度。支持通过查询参数覆盖：<code className="rounded bg-gray-100 px-1 py-0.5 dark:bg-white/[0.06]">?reasoningEffort=high</code>。
                  </div>
                </div>
              )}
                  ...
                  {!localAutoSaveSupported ? (
                    ...
                    <>
                      ...
                      <div className="grid gap-2 sm:grid-cols-2">
                        ...
                        <button
                          type="button"
                          onClick={() => { void handleRetryLocalAutoSaves() }}
                        ...
                        >
                          {localAutoSaveNeedsPermission
                            ? `重新授权并补保存（${pendingLocalAutoSaveCount}）`
                            : pendingLocalAutoSaveCount > 0
