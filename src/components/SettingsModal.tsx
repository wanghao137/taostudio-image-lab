                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 text-xs leading-relaxed text-emerald-700/80 dark:text-emerald-200/80">
                        YDN 当前 <code className="rounded bg-white/70 px-1 py-0.5 dark:bg-black/20">/v1/images/generations</code> 通道会返回上游 multipart 解析错误；实测 <code className="rounded bg-white/70 px-1 py-0.5 dark:bg-black/20">/v1/responses</code> + <code className="rounded bg-white/70 px-1 py-0.5 dark:bg-black/20">{YDN_IMAGE_MODEL}</code> 可稳定生图。推荐使用 Responses API、GPT Image 兼容参数、开启同源 API 代理并关闭流式；为降低 4K 生成的超时和限流风险，YDN 会按单图任务提交。兼容参数可以手动关闭，提交时不会被强制改回。
                      </p>
                    </div>
