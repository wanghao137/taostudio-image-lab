...
export default function InputBar() {
  ...
  const qualityOptions = [
    ...
    // gpt-image-2.5 专属质量档；其他模型提交前会自动降为 high（paramCompatibility）。
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
    // 兼容部分输入法：用 Enter 确认候选字时会额外派发 Enter keydown，
  ...
  return (
    <>
      ...
      <div data-input-bar className="fixed bottom-3 left-1/2 z-30 w-full max-w-5xl -translate-x-1/2 px-3 transition-all duration-300 sm:bottom-5 sm:px-4">
        ...
        <div ref={cardRef} className="rounded-2xl border border-stone-200/80 bg-white/82 p-3 shadow-[0_18px_60px_rgba(40,30,20,0.16)] ring-1 ring-white/80 backdrop-blur-2xl dark:border-white/[0.08] dark:bg-[#16120f]/88 dark:shadow-[0_18px_70px_rgba(0,0,0,0.45)] dark:ring-white/[0.04] sm:p-4">
          {/* 移动端拖动条 */}
          <div
            ref={handleRef}
