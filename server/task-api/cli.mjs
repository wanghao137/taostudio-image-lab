...
const api = await createTaskApi({
  ...
  concurrency: Number(value('IMAGE_TASK_API_CONCURRENCY', '2')),
  providerTimeoutMs: Number(value('IMAGE_TASK_API_PROVIDER_TIMEOUT_MS', '180000')),
  providerConfig: {
...
console.log(`Bearer token: ${api.token}`)
